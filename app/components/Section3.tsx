    ...
    <div id="problem" ref={containerRef} className="relative w-full font-[family:var(--font-jakarta)] bg-[#F4F3EE] lg:h-[250vh]">
      ...
      >
        ...
        >
          ...
          <div className="absolute bottom-6 sm:bottom-8 md:bottom-16 left-5 sm:left-8 md:left-16 right-5 sm:right-8 md:right-16 z-20 text-white pointer-events-none">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 md:mb-5">
              <div className="h-1 sm:h-1.5 w-8 sm:w-10 md:w-14 bg-[#F28F3B] rounded-full" />
              <span className="text-[8px] sm:text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-white/80">Click to Play with Sound</span>
            </motion.div>
            <motion.h3 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black uppercase tracking-tighter leading-[1] sm:leading-[0.9]">
              Saving Food,<br /><span className="text-[#F28F3B]">Empowering Locals.</span>
            </motion.h3>
          </div>
